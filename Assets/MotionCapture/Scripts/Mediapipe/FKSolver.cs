...
namespace MotionCapture.Mediapipe
{
    public class FKSolver : Core.FKSolver
    {
        ...
        public override void Apply(PoseData pose)
        {
            ...
            var nose = GetBonePosition(pose, Bones.nose);
            var left_eye = GetBonePosition(pose, Bones.left_eye);
            var right_eye = GetBonePosition(pose, Bones.right_eye);
            var left_ear = GetBonePosition(pose, Bones.left_ear);
            var right_ear = GetBonePosition(pose, Bones.right_ear);
            var mouth = (GetBonePosition(pose, Bones.left_mouth) + GetBonePosition(pose, Bones.right_mouth)) / 2;

            var left_inner_hand = GetBonePosition(pose, Bones.left_inner_hand);
            var left_index_distal = GetBonePosition(pose, Bones.left_hand_tip);
            var left_outer_hand = GetBonePosition(pose, Bones.left_outer_hand);

            var right_inner_hand = GetBonePosition(pose, Bones.right_inner_hand);
            var right_index_distal = GetBonePosition(pose, Bones.right_hand_tip);
            var right_outer_hand = GetBonePosition(pose, Bones.right_outer_hand);

            if (IsMirror)
            {
                left_hip = GetBonePosition(pose, Bones.right_hip);
                left_hip.x = -left_hip.x;
                right_hip = GetBonePosition(pose, Bones.left_hip);
                right_hip.x = -right_hip.x;
                left_shoulder = GetBonePosition(pose, Bones.right_shoulder);
                left_shoulder.x = -left_shoulder.x;
                right_shoulder = GetBonePosition(pose, Bones.left_shoulder);
                right_shoulder.x = -right_shoulder.x;
                left_elbow = GetBonePosition(pose, Bones.right_elbow);
                left_elbow.x = -left_elbow.x;
                right_elbow = GetBonePosition(pose, Bones.left_elbow);
                right_elbow.x = -right_elbow.x;
                left_wrist = GetBonePosition(pose, Bones.right_wrist);
                left_wrist.x = -left_wrist.x;
                right_wrist = GetBonePosition(pose, Bones.left_wrist);
                right_wrist.x = -right_wrist.x;
                left_knee = GetBonePosition(pose, Bones.right_knee);
                left_knee.x = -left_knee.x;
                right_knee = GetBonePosition(pose, Bones.left_knee);
                right_knee.x = -right_knee.x;
                left_ankle = GetBonePosition(pose, Bones.right_ankle);
                left_ankle.x = -left_ankle.x;
                right_ankle = GetBonePosition(pose, Bones.left_ankle);
                right_ankle.x = -right_ankle.x;
                left_toe = GetBonePosition(pose, Bones.right_toe);
                left_toe.x = -left_toe.x;
                right_toe = GetBonePosition(pose, Bones.left_toe);
                right_toe.x = -right_toe.x;
                left_heel = GetBonePosition(pose, Bones.right_heel);
                left_heel.x = -left_heel.x;
                right_heel = GetBonePosition(pose, Bones.left_heel);
                right_heel.x = -right_heel.x;

                nose = GetBonePosition(pose, Bones.nose);
                nose.x = -nose.x;
                left_eye = GetBonePosition(pose, Bones.right_eye);
                left_eye.x = -left_eye.x;
                right_eye = GetBonePosition(pose, Bones.left_eye);
                right_eye.x = -right_eye.x;
                left_ear = GetBonePosition(pose, Bones.right_ear);
                left_ear.x = -left_ear.x;
                right_ear = GetBonePosition(pose, Bones.left_ear);
                right_ear.x = -right_ear.x;
                mouth = (GetBonePosition(pose, Bones.right_mouth) + GetBonePosition(pose, Bones.left_mouth)) / 2;
                mouth.x = -mouth.x;

                left_inner_hand = GetBonePosition(pose, Bones.right_inner_hand);
                left_inner_hand.x = -left_inner_hand.x;
                left_index_distal = GetBonePosition(pose, Bones.right_hand_tip);
                left_index_distal.x = -left_index_distal.x;
                left_outer_hand = GetBonePosition(pose, Bones.right_outer_hand);
                left_outer_hand.x = -left_outer_hand.x;

                right_inner_hand = GetBonePosition(pose, Bones.left_inner_hand);
                right_inner_hand.x = -right_inner_hand.x;
                right_index_distal = GetBonePosition(pose, Bones.left_hand_tip);
                right_index_distal.x = -right_index_distal.x;
                right_outer_hand = GetBonePosition(pose, Bones.left_outer_hand);
